...
class PullRefreshScrollView extends BaseScrollComponent {
    constructor(args) {
        super(args);
    ...
    render() {
        ...
        return (<Scroller ref={(scrollView) => {
            ...
            }} bounces={this.props.onRefresh ? true : false} onScrollEndDrag={(e) => this.onScrollEndDrag(e)} onScrollBeginDrag={() => this.onScrollBeginDrag()} removeClippedSubviews={false} scrollEventThrottle={16} {...this.props} horizontal={this.props.isHorizontal} onScroll={this._onScroll} onLayout={!this._isSizeChangedCalledOnce || this.props.canChangeSize
                ? this._onLayout
                : this._dummyOnLayout}>
    ...
    renderBottomContent() {
        ...
        jsx.push(<Text key={2} style={{ color: '#979aa0' }}>
    ...
    _onLayout(event) {
        ...
        if (this._height !== event.nativeEvent.layout.height ||
...
const dateFormat = function (dateTime, fmt) {
    ...
    for (let k in o) {
        if (new RegExp('(' + k + ')').test(tmp)) {
            tmp = tmp.replace(RegExp.$1, RegExp.$1.length === 1
                ? o[k]
