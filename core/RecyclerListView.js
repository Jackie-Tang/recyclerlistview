...
const IS_WEB = Platform.OS === 'web',
  refreshRequestDebouncer = debounce((executable) => {
    executable();
  });
  ...
  _checkAndChangeLayouts(newProps, forceFullRender) {
    ...
    if (
      ...
      this._virtualRenderer.setLayoutManager(
        new LayoutManager(newProps.layoutProvider, this._layout, newProps.isHorizontal),
      );
    ...
    if (!ObjectUtil.isNullOrUndefined(dataIndex) && dataIndex < dataSize) {
      ...
      return (
        <ViewRenderer
          ...
          itemAnimator={Default.value(this.props.itemAnimator, this._defaultItemAnimator)}
  ...
  _processOnEndReached() {
    if (this.props.onEndReached && this._virtualRenderer) {
      const layout = this._virtualRenderer.getLayoutDimension(),
        windowBound = this.props.isHorizontal
